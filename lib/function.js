...
const getTime = (format, date) => {
	if (date) {
		...
		return moment.tz('Lagos/Nigeria').locale('en-IN').format(format)
	}
}
...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./ImmanuelxMedia/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./ImmanuelxMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./ImmanuelxMedia/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./ImmanuelxMedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./ImmanuelxMedia/trash/${filename}.mp4`), unlink(`./ImmanuelxMedia/trash/${filename}.gif`)])
    return buffer5
}
